...
public class Main {
	// main 메서드
	// : 해당 메서드가 존내해야 자바 프로그램 실행이 가능 (콘솔)
	public static void main(String[] args) {
		...
		// ===Book 클래스===
		// 책 제목(title), 저자(author), 택교유번호(isbn), 대여가능여부(isAvailable)
		...
		} catch (BookAlredyBorrowedException e) {
			System.out.println(e.getMessage()); // Hi(은)는 이미 대여 중입니다.
		}
		
		System.out.println("제목으로 Hi 검색");
		List<Book> foundBooksByTitle = library.searchBooksByTitle("Hi");
		for (Book book: foundBooksByTitle) {
			System.out.println(book.getTitle() + " / " + book.getAuthor());
		}
		System.out.println("저자  이승아 검색");
		List<Book> foundBooksByAuthor = library.searchBooksByAuthor("Hi");
