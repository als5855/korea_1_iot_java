...
// == 익명 클래스 == 
// : 클래스를 정의 하면서 동시에 객체를 생성하는 방법
//  > 이름이 없기 때문에 재사용 X, 일회성 사용 객체 정의에 유용

// === 익명클래스의 특징 ===
// 1) 한번만 사용하는 인터페이스 구현, 추상 클래스 구현
// 2) 특정 로직을 캡술화 - 내부 클래스의 일종
// 3) 명시적인 생성자를 가질 수X
...
// 1. 인터페이스 구현 예시
interface Animal {
	void sound(); // 추상메서드 - 구현이 없는 메서드
	// +) 익명 클래스에서의 인터페이스는함수형 인터페이스로 
...
public class Anonymous {
		public static void main(String[] args) {
			...
			System.out.println(person.getName()); // 장지민
		}
}
