...
public class Main {
		public static void main(String[] args) {
			...
			bookController.addBook("자바 한 번에 끝내기", "장지민", 2023);
			bookController.addBook("자바스크립트 한 번에 끝내기", "이지윤", 2022);
			bookController.addBook("리액트 한 번에 끝내기", "이미진", 2023);
			bookController.addBook("SQL 한 번에 끝내기", "서지연", 2023);
			...
			System.out.println("==특정한 id 도서 수정==");
			bookController.updateBook(2, "스프링 한번에 끝내기", "장지민", 2024);
			System.out.println("수정 후 도서 :" + bookController.getBook(2));
			
			System.out.println("==특정 id 도석 삭제==");
			bookController.deleteBook(2);
			bookController.getAllBooks().forEach(System.out::println);
			
			System.out.println("--추가 후 전체 출력");
			bookController.addBook("안녕하세요", "이민지", 2022);
			bookController.getAllBooks().forEach(System.out::println);
		}
}
